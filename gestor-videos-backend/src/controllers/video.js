...
const getVideosByGroup = async (req, res, next) => {
  try {
    ...
    if (groupId == "")
      return res.status(404).json({ message: "Group not found", videos: [] });
...
const getVideo = async (req, res, next) => {
  try {
    ...
    const videoFound = await videoSchema.findById(videoId).populate("owner");
    ...
    if (groupId != "") {
      ...
      for (const groupUser of group.users) {
        ...
        if (groupUser.sendNotification) {
          const linkUrl = "http://localhost:4200/watch/" + newVideo._id;
          const content =
          ...
          const savedNotification =
            await notificationController.saveNotification(content, receiver);
